...
const TeamPage: React.FC = () => {
  ...
  const fetchTeams = useCallback(async () => {
    try {
      ...
      const res = await axiosInstance.get<Team[]>(`/api/teams/my-teams`, {
  ...
  const handleDeleteTeam = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete team "${selectedTeam.name}"?`))
      return;

    try {
      await axiosInstance.delete(
        //`http://localhost:8080/api/teams/${selectedTeam.id}/delete`,
        `/api/teams/${selectedTeam.id}/delete`,
        {
          params: { performedBy: loggedInUserId },
        }
      );
      ...
      if (updatedTeams.length > 0) {
        ...
        localStorage.setItem("selectedTeam", JSON.stringify(updatedTeams[0]));
      } else {
  ...
  const handleInvite = async () => {
    ...
    try {
      setInviting(true);
      await axiosInstance.put(`${API_URL}/api/teams/${selectedTeam.id}/invite`, null, {
  ...
  const handleChangeRole = async (member: TeamMember) => {
    ...
    try {
      await axiosInstance.put(`${API_URL}/api/teams/${selectedTeam.id}/change-role`, null, {
  ...
  const handleRemove = async (member: TeamMember) => {
    ...
    try {
      await axiosInstance.delete(`${API_URL}/api/teams/${selectedTeam.id}/remove-member`, {
  ...
  const handleAcceptInvite = async (member: TeamMember) => {
    if (!selectedTeam) return;
    try {
      await axiosInstance.put(`${API_URL}/api/teams/${selectedTeam.id}/accept`, null, {
        params: { userId: loggedInUserId },
      });
      fetchMembers(selectedTeam.id);
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div className="flex gap-3 flex-wrap">
            {teams.map((team) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg border ${selectedTeam?.id === team.id ? "bg-blue-600 text-white" : "bg-white"
                  }`}
              >
                {team.name}
              </button>
          ...
          {isAdmin && selectedTeam && (
            <button
              ...
              className="px-4 py-2 border rounded-lg bg-white hover:bg-gray-100 text-red-600"
            >
              Delete Team
            </button>
          )}
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex gap-1 mt-1 flex-wrap">
                          ...
                          <span
                            className={`text-xs px-2 py-1 rounded ${m.status === "ACTIVE"
