...
const Projects: React.FC = () => {
  ...
  const API_BASE = "/projects";   // for nignix
  ...
  const fetchProjects = useCallback(async () => {
    try {
      const res = await axios.get<ProjectResponse[]>(
        `${API_URL}${API_BASE}/getAllProjectsWithProgress`
      );
            ...
            >
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded ${project.status === "Completed"
                ...
                <div className="mt-2 space-y-1 max-h-40 overflow-y-auto">
                  {project.tasks.map((task) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-xs px-2 py-1 rounded ${task.status === "DONE"
                          ? "bg-green-100 text-green-700"
